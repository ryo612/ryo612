void UI() {
  ...
  for (int i=0; i<tImage.length; i++) {
    rect(Tx, Ty+(Th)*i, Tw, Th);
    if (i<tImage.length) {
      image(tImage[i], Tx, Ty+(Th)*i, Tw, Th);
    }
  }
  noStroke();
  fill(0, 50);
  rect(Tx, Ty+Th*tool, Tw, Th);

  //file
  for (int i=0; i<2; i++) {
    stroke(0);
    noFill();
    rect(Fx+(Fw)*i, Fy, Fw, Fh);
    String sf;
    if (i==0) {
      sf="save";
    } else {
  ...
  noStroke();
  fill(0, 50);
  rect(Px+Pw*(selectCol%7), Py+Ph*(selectCol/7), Pw, Ph);
  for (int j=0; j<2; j++) {
    for (int i=0; i<7; i++) {
      noFill();
      stroke(0);
  ...
  textAlign(CENTER);
  textSize(14);
  text("Size", Px+40, PSy+5);
  text(nf(lensD/zoom,1,1), PSx+dm*((lensD/zoom)/0.5)-dm*2, PSy-20);
